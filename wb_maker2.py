...
def write_data_to_file(filename, data):
    with open(filename, 'w') as file:
        for i, entry in enumerate(data):
            file.write(f"conv2_bias[{(i+1)*8-1}:{i*8}] <= 8'b{int(entry['bias']):08b}; //normalized bias={entry['bias']:.4f}\n")
            ...
            file.write(f"conv2_weight[{(i+1)*1152-1}:{i*1152}] <= 1152'b{weights_str};\n")


# 파일에서 데이터 읽어오기
filename = 'conv2Layer.txt'  # 실제 파일 이름으로 변경해주세요
data = read_data_from_file(filename)

# 데이터 출력
#rearrange_and_print_data(data)

filename_write = 'WB2.txt'  # 결과를 저장할 파일 이름으로 변경해주세요
